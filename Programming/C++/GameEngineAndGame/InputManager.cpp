...
void
InputManager::unbind(const std::string& action, const int player_idx)
{
	FF_ASSERT_MSG(player_idx < playerCount, "player out of bounds");
	m_actionBinding[player_idx].erase(action);
}

int
InputManager::getKeyForAction(const std::string& action, const int player_idx)
{
	FF_ASSERT_MSG(player_idx < playerCount, "player out of bounds");

	const auto it = m_actionBinding[player_idx].find(action);
	if (it != m_actionBinding[player_idx].end())
	{
		return it->second;
	}
	return 0;
